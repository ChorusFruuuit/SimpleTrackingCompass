...
public final class PlayerUtils {
    public static void playerNotFoundError(String name) {
        if (client().player == null || client().player.getGameProfile().getName().equals(name)) return;
    ...
    public static <P extends Player> P getNearestPlayer(P centerPlayer, List<P> playerList) {
        ...
        for (P player : playerList) {
            if (player.getUUID().equals(centerPlayer.getUUID()) || !player.level().dimension().location().equals(centerPlayer.level().dimension().location()))
                continue;
    ...
    public static boolean isValidOnlinePlayer(GameProfile gameProfile) {
        return getOnlinePlayer(gameProfile.getId()) != null
            && !gameProfile.getName().isEmpty()
            // Standard minecraft name regex (no length matching)
            && gameProfile.getName().matches("^[a-zA-Z0-9_]+$");
    }
    ...
    public static void setOverlayMessage(Component message, int ticks) {
        ...
        guiAccessor.setOverlayMessageString(message);
        guiAccessor.setOverlayMessageTime(ticks);
    }
}
