...
import net.chorus.simpletrackingcompass.screen.TargetSelectorScreen;import net.chorus.simpletrackingcompass.util.PlayerUtils;
...
public final class ClientEvents {
    public static void onClientEntityLoad(Entity entity) {
        ...
        if (CompassTracker.getTarget().getGameProfile().id().equals(player.getUUID())) {
    ...
    public static void onClientJoin() {
        ...
        if (!ConfigManager.hasSeenGuideMessage && client().player != null) {
            client().gui.getChat().addClientSystemMessage(PlayerUtils.STCGuideMessage());
