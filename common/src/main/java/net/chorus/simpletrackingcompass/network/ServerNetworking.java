...
public final class ServerNetworking {
    private static BiPredicate<ServerGamePacketListenerImpl, Identifier> channelPresenceTest =
        (_, _) -> false; // Default implementation. To be overridden by mod loader.
    ...
    public static void onPlayerPositionRequestReceive(PlayerPositionRequest payload, ServerPlayer requester) {
        ...
        ServerPlayer player = playerUUID.isPresent()
            ? (ServerPlayer) requester.level().getPlayerInAnyDimension(playerUUID.get())
            : PlayerUtils.getNearestPlayer(requester, requester.level().getServer().getPlayerList().getPlayers());
    ...
    public static void sendClientboundPacket(CustomPacketPayload payload, ServerPlayer packetReceiver) {
        ...
        if (!canSend(packetReceiver.connection, payload.type().id())) {
            LOGGER.warn(
                ...
                payload.type().id(), packetReceiver.getPlainTextName()
            );
            return;
        }

        LOGGER.debug("Verification complete. Sending {} to {}.", payload.type().id(), packetReceiver.getPlainTextName());
        try {
            packetReceiver.connection.send(new ClientboundCustomPayloadPacket(payload));
        } catch (Throwable t) {
            LOGGER.warn("The send of {} to {} failed!", payload.type().id(), packetReceiver.getPlainTextName(), t);
        }
    }

    public static void setChannelPresenceTest(BiPredicate<ServerGamePacketListenerImpl, Identifier> channelPresenceTest) {
        ServerNetworking.channelPresenceTest = channelPresenceTest;
    }

    public static boolean canSend(ServerGamePacketListenerImpl packetListener, Identifier channelId) {
