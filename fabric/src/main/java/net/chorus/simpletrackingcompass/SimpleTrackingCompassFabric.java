...
public class SimpleTrackingCompassFabric implements ModInitializer, ClientModInitializer {
    ...
    public void onInitialize() {
        PayloadTypeRegistry.serverboundPlay().register(PlayerPositionRequest.TYPE, PlayerPositionRequest.CODEC);
        PayloadTypeRegistry.clientboundPlay().register(PlayerPositionResponse.TYPE, PlayerPositionResponse.CODEC);
        ...
        ServerNetworking.setChannelPresenceTest((packetListener, _) ->
            packetListener != null
        );
    }
    ...
    public void onInitializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(
            ...
            (payload, _) -> ClientNetworking.onPlayerPositionResponseReceive(payload)
        );

        ClientNetworking.setChannelPresenceTest(channelName ->
            client().getConnection() != null && ClientPlayNetworking.canSend(channelName)
        );

        ClientTickEvents.END_CLIENT_TICK.register(
            _ -> CompassTracker.onEndTick()
        );
        HudElementRegistry.addFirst(
            COMPASS, (graphics, _) -> CompassTracker.extractRenderState(graphics)
        );
        ...
        ClientEntityEvents.ENTITY_LOAD.register(
            (entity, _) -> ClientEvents.onClientEntityLoad(entity)
        );
        ClientEntityEvents.ENTITY_UNLOAD.register(
            (entity, _) -> ClientEvents.onClientEntityUnload(entity)
        );

        ClientPlayConnectionEvents.JOIN.register(
            (_, _, _) -> ClientEvents.onClientJoin()
        );
        ClientPlayConnectionEvents.DISCONNECT.register(
            (_, _) -> ClientEvents.onClientDisconnect()
        );
        ClientLifecycleEvents.CLIENT_STOPPING.register(
            _ -> ClientEvents.onClientStopping()
        );

        ScreenEvents.AFTER_INIT.register(
            (_, screen, _, _) -> {
                TexturedButton compassButton = CompassButtonFactory.afterScreenInit(screen);
                if (compassButton != null) Screens.getWidgets(screen).add(compassButton);
            }
        );
    }
}
